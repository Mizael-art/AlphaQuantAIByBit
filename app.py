...
def run_analysis(
    ...
    # Piso absoluto: abaixo disso não dá pra calcular NADA com sentido
    # (RSI de 1-2 candles não tem variação suficiente pra significar
    # qualquer coisa -- é indefinição matemática, não "baixa
    # confiança"). 5 é o mínimo pro swing detection (lookback=2)
    # sequer identificar um único swing.
    #
    # Entre 5 e 200 candles: a análise roda INTEIRA (todos os
    # indicadores, estrutura, score) com o histórico disponível --
    # EMA/RSI/ATR/MACD (`indicators/*.py`) ajustam `min_periods` ao
    # que existe em vez de virar NaN. Isso é intencional para não
    # descartar símbolos recém-listados na Bybit (podem ser boas
    # oportunidades cedo) -- mas o resultado é sinalizado como
    # `data_quality.reduced_confidence=True` pra quem consome o JSON
    # saber que aquele número é menos maduro que uma análise com
    # histórico completo (ex.: uma "EMA200" calculada com 35 candles
    # não convergiu como uma EMA200 de verdade).
    if candles_available < 5:
        raise InsufficientDataError(
            f"Candles insuficientes para {symbol} ({timeframe}): "
            f"{candles_available} recebidos, mínimo absoluto é 5 "
            "(abaixo disso os indicadores não têm sentido matemático)."
        )

    IDEAL_CANDLES = 200
    reduced_confidence = candles_available < IDEAL_CANDLES
    data_quality = {
        "candles_used": candles_available,
        "ideal_candles": IDEAL_CANDLES,
        "reduced_confidence": reduced_confidence,
        "note": (
            f"Histórico parcial ({candles_available}/{IDEAL_CANDLES} candles) -- "
            "provavelmente ativo recém-listado. Indicadores de período longo "
            "(ex. EMA200) ainda não convergiram totalmente; trate o score "
            "e a estrutura aqui com confiança reduzida em relação a um "
            "símbolo com histórico completo."
