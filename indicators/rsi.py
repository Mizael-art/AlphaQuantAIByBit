...
def calculate_rsi(series: pd.Series, period: int | None = None) -> pd.Series:
    ...
    # min_periods travado em len(series) (não no period fixo) pelo mesmo
    # motivo do EMA -- ver docstring de indicators.ema.calculate_ema.
    min_periods = min(period, max(int(gains.notna().sum()), 1))
    avg_gain = gains.ewm(alpha=1 / period, min_periods=min_periods, adjust=False).mean()
    avg_loss = losses.ewm(alpha=1 / period, min_periods=min_periods, adjust=False).mean()
