...
def calculate_ema(series: pd.Series, period: int) -> pd.Series:
    ...
    Nota sobre séries curtas (símbolos recém-listados): `min_periods`
    é limitado a `len(series)` em vez do `period` fixo -- isso evita
    que uma EMA200 vire uma coluna inteira de NaN quando o ativo só
    tem, digamos, 40 candles de histórico. O valor resultante ainda é
    calculado (não é NaN), mas é uma aproximação pouco convergida --
    ver `app.run_analysis`, que anota `data_quality.reduced_confidence`
    no resultado sempre que isso acontece, pra quem consome o JSON
    saber que aquele número é menos confiável que uma EMA200 "de
    verdade" com histórico completo.
    """
    min_periods = min(period, max(int(series.notna().sum()), 1))
