...
def calculate_atr(df: pd.DataFrame, period: int | None = None) -> pd.Series:
    ...
    # min_periods travado em len(df) -- ver docstring de indicators.ema.calculate_ema.
    min_periods = min(period, max(int(true_range.notna().sum()), 1))
    atr = true_range.ewm(alpha=1 / period, min_periods=min_periods, adjust=False).mean()
