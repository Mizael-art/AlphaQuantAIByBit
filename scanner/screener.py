...
def _scan_one(
    ...
    # Selo de confiança: pega o pior caso entre HTF/LTF (o que tiver
    # menos histórico manda) -- ver `app.run_analysis`/`data_quality`.
    dq_htf = result_htf.data_quality or {}
    dq_ltf = result_ltf.data_quality or {}
    reduced_confidence = bool(dq_htf.get("reduced_confidence")) or bool(dq_ltf.get("reduced_confidence"))
    data_quality_note = ""
    if reduced_confidence:
        worst = dq_htf if dq_htf.get("candles_used", 10**9) <= dq_ltf.get("candles_used", 10**9) else dq_ltf
        data_quality_note = (
            f"Histórico parcial ({worst.get('candles_used')}/{worst.get('ideal_candles')} candles) -- "
            "provável ativo recém-listado. Score/estrutura com confiança reduzida."
        )
...
def get_universe_shortlist(
    top_n: int = SCAN_STAGE1_TOP_N,
    min_turnover_usdt: float = SCAN_STAGE1_MIN_TURNOVER_USDT,
) -> tuple[list[str], int]:
    """
    Stage 1 isolado (sem Stage 2), reaproveitado tanto por
    `scan_universe` quanto por `/discovery/top-trades?universe=all_bybit`
    (`discovery/engine.py`) -- mesma fonte de dados, mesmo filtro de
    liquidez/atividade, sem duplicar a lógica de ranking.

    Returns:
        (shortlist ordenada por atividade, tamanho total do universo
        Bybit antes do filtro -- útil para observabilidade/transparência,
        ver Documento Master seção 30).

    Raises:
        UniverseUnavailableError: mesma condição de `scan_universe`.
    """
    universe = get_all_bybit_usdt_perpetuals()
    tickers = get_bulk_ticker_snapshot()
    stage1_candidates = rank_candidates(
        tickers=tickers, universe=universe, top_n=top_n, min_turnover_usdt=min_turnover_usdt
    )
    return [c.symbol for c in stage1_candidates], len(universe)
