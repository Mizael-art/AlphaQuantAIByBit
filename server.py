...
from scanner.screener import get_universe_shortlist, scan_market, scan_universe
...
def _start_background_scan_loop() -> None:
    """
    DESLIGADO a pedido do usuário (rodava 24/7 escaneando o universo
    inteiro sem parar -- ver `scanner/background_loop.py` para a
    implementação, que continua existindo caso queira reativar no
    futuro). Agora o scan só roda quando alguém chama `/scan` ou
    `/scan?universe=all_bybit` de fato -- sem processo de fundo
    consumindo Bybit sozinho o dia inteiro.

    Efeito colateral: `/scan/latest` (que lê o resultado do loop de
    fundo) sempre vai devolver 503 agora, porque nenhum ciclo roda
    mais pra alimentar `ScanSnapshot`. Use `/scan?universe=all_bybit`
    diretamente em vez de `/scan/latest` -- é mais lento por request
    (calcula na hora), mas não deixa nada rodando sozinho.

    Pra reativar o loop de fundo: descomente a chamada abaixo.
    """
...
def get_scan_latest(
    ...
    """AVISO: loop de fundo desligado (ver server.py) -- este endpoint sempre retorna 503 agora. Use /scan?universe=all_bybit diretamente para o scan completo do universo Bybit."""
...
def get_top_trades(
    symbols: str | None = Query(
        default=None,
        description=f"Símbolos separados por vírgula (padrão: watchlist de {len(DEFAULT_SCAN_SYMBOLS)} ativos em config.DEFAULT_SCAN_SYMBOLS). Ignorado se universe='all_bybit'.",
    ),
    universe: str = Query(
        default="watchlist",
        description="'watchlist' (padrão, usa `symbols` ou DEFAULT_SCAN_SYMBOLS) | 'all_bybit' (varre todo o universo Bybit via filtro de liquidez/atividade antes do Multi-Score -- ver stage1_top_n).",
    ),
    stage1_top_n: int = Query(
        default=SCAN_STAGE1_TOP_N,
        description="Só usado com universe='all_bybit': quantos candidatos do pré-filtro de liquidez recebem o pipeline completo (regime+Multi-Score+correlação).",
    ),
    stage1_min_turnover_usdt: float = Query(
        default=SCAN_STAGE1_MIN_TURNOVER_USDT,
        description="Só usado com universe='all_bybit': piso de turnover 24h (USDT) do pré-filtro de liquidez.",
    ),
    ...
    """Varre símbolos (watchlist fixa OU todo o universo Bybit via universe='all_bybit'), detecta regime, aplica regime-first, calcula Multi-Score e Correlated Exposure Engine. Retorna top_n oportunidades e ativos sem edge, com motivo. Scores não são probabilidade."""
    if universe not in ("watchlist", "all_bybit"):
        raise HTTPException(status_code=422, detail="universe deve ser 'watchlist' ou 'all_bybit'.")
    if universe == "all_bybit" and symbols is not None:
        raise HTTPException(
            status_code=422,
            detail="symbols e universe='all_bybit' são mutuamente exclusivos -- escolha um dos dois modos de seleção.",
        )

    universe_size: int | None = None
    if universe == "all_bybit":
        try:
            symbol_list, universe_size = get_universe_shortlist(
                top_n=stage1_top_n, min_turnover_usdt=stage1_min_turnover_usdt
            )
        except UniverseUnavailableError as exc:
            raise HTTPException(status_code=502, detail=f"Falha ao obter universo Bybit: {exc}") from exc
    else:
        symbol_list = (
            [s.strip().upper() for s in symbols.split(",") if s.strip()]
            if symbols
    ...
    # Transparência (Documento Master seção 30/31): quantos ativos entraram
    # em cada etapa do funil, não só o resultado final.
    result["universe_mode"] = universe
    if universe == "all_bybit":
        result["universe_size"] = universe_size
        result["stage1_candidates"] = len(symbol_list)
        result["stage1_min_turnover_usdt"] = stage1_min_turnover_usdt
    return result
